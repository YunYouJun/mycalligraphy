...
function test1()
{
	cxt.drawImage(imgdata,0,0,234*3,138*3);
}
...
//		i=i+4;i=(x*pic_width+y)*4
	...
	cxt.putImageData(imgaction,0,0);}catch (e) {alert(e.message)} 
}
function cleandata()
{
	...
	cxt.putImageData(imgaction,0,0);
}
function runAction()
{
//	alert("flag:"+_flag);
...
function ex()
{
//	alert("ex:___x:"+_cx+"___y:"+_cy+"___NL:"+_nl+"str.length:"+_str.length);
	if(_nl>_str.length)
	{
		_fle=1;
	}
...
function startAction()
{
	...
	img_i=0;
	img_n=1;
	imgaction=cxt.createImageData(pic_width*img_n,pic_height*img_n);	
	_flag=1;
...
{
	cxt.clearRect(0,0,pic_width,pic_height);
...
function initallvalue()
{
	...
	pic_width=560;
	pic_height=360;
	cxt.drawImage(imgdata,0,0);
	 try {
		imgactdata=cxt.getImageData(0,0,pic_width,pic_height);
...
//	cxt.putImageData(imgactdata,0,0);
		cleanCanvas();
	     }catch (e) {alert(e.message)}  
...
function test5()
{
	imgaction=cxt.createImageData(56,36);
	var i;
	var j;
	for(i=1;i<56;i++)
	{
